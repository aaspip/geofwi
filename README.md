...
<img src='https://github.com/aaspip/gallery/blob/main/geofwi/DiffusionModel_SGDS_Marmousi.jpg' alt='comp' width=960/>

<img src='https://github.com/aaspip/gallery/blob/main/geofwi/DiffusionModel_SGDS_Overthrust.jpg' alt='comp' width=960/>

# Below is a benchmark example comparing SeisInvNet with another data-driven FWI approach (InversionNet, Wu and Lin, 2019). The benchmark is conducted by using 2000 samples from the GeoFWI dataset for training and 500 samples for independent testing. It is clear that SeisInvNet performs better than InversionNet on the GeoFWI dataset, e.g., fewer spurious structures (like the fake salts on the third row), and more accurately recovered layered structures.
<img src='https://github.com/aaspip/gallery/blob/main/geofwi/seisinvnet_and_inversionnet' alt='comp' width=960/>

# Augmented GeoFWI with more realistic salt intrusion

## GeoFWI-Augmented extends the original GeoFWI dataset with 20,000 procedurally generated salt-bearing velocity models, comprising 10,000 examples from each of two new structural families. The first family contains snaking salt diapirs constructed from connected ellipsoidal segments with curved stems and multiple salt heads, while the second contains broad, irregular salt bodies formed by combining three to six randomly positioned, scaled, and rotated ellipsoids. Each salt body is embedded in a randomized folded sedimentary background containing one to seven layers. Three dimensional 100*100*100 models are generated first, and two dimensional 100*100 slices are then extracted along both horizontal directions. Only slices containing a meaningful high velocity salt feature are retained, producing a final augmented dataset of 69,476 velocity models.

<img src='https://github.com/aaspip/gallery/blob/main/geofwi/SaltAug.png' alt='comp' width=960/>
<img src='https://github.com/aaspip/gallery/blob/main/geofwi/Salt2Aug.png' alt='comp' width=960/>

The above two figures can be reproduced using 
