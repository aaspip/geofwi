...
# Below is an example for applying the diffusion model trained from the GeoFWI dataset to a completely independent Marmousi model. It can be observed that 1) L2-only FWI suffers from cycle skipping and artifacts; 2) L2 + TV FWI improves continuity but over-smooths the structure; 3) L2 + SGDS (Split Gibbs Diffusion Sampling) FWI recovers high-resolution features like salt bodies and faults, closely matching the ground truth.
